"""
Servicio de Tokens JWT - Principio de Responsabilidad Única
Ahora con RS256: firma con clave privada, verifica con clave pública.
Incluye iat (issued at) y jti (JWT ID) para trazabilidad de tokens.
"""
import uuid
from datetime import datetime, timedelta, timezone
from typing import Dict, Any, Optional, Tuple
from jose import JWTError, jwt
from app.core.config import settings
from app.core.crypto import get_private_key, get_public_key
from app.interfaces.auth.ITokenService import ITokenService


class TokenService(ITokenService):
    """
    Implementación concreta del servicio de tokens JWT.
    Maneja solo la creación, validación y revocación de tokens.
    """

    def __init__(self):
        self.algorithm = settings.algorithm  # "RS256"
        self.access_token_expire_minutes = settings.access_token_expire_minutes
        # Claves RSA: privada para firmar, pública para verificar
        self._private_key = get_private_key()
        self._public_key = get_public_key()
        # En producción, usar Redis o base de datos para blacklist
        self._blacklisted_tokens: set = set()

    async def create_access_token(
        self, data: Dict[str, Any], expires_delta: Optional[int] = None
    ) -> Tuple[str, datetime]:
        """
        Crea un token de acceso JWT firmado con RSA (RS256).
        Ahora incluye:
        - iat (issued at): timestamp de emisión
        - jti (JWT ID): UUID único para identificar este token individualmente
        """
        to_encode = data.copy()

        now = datetime.now(timezone.utc)
        if expires_delta:
            expire = now + timedelta(seconds=expires_delta)
        else:
            expire = now + timedelta(minutes=self.access_token_expire_minutes)

        to_encode.update({
            "exp": expire,
            "iat": now,
            "jti": str(uuid.uuid4()),
        })
        encoded_jwt = jwt.encode(
            to_encode,
            self._private_key,
            algorithm=self.algorithm,
            headers={"kid": "authcore-key-1"},
        )

        return encoded_jwt, expire

    async def verify_token(self, token: str) -> Optional[Dict[str, Any]]:
        """
        Verifica y decodifica un token JWT usando la clave pública.
        """
        try:
            if await self.is_token_revoked(token):
                return None

            payload = jwt.decode(
                token, self._public_key, algorithms=[self.algorithm]
            )
            return payload
        except JWTError:
            return None

    async def refresh_token(
        self, refresh_token: str
    ) -> Optional[Tuple[str, datetime]]:
        payload = await self.verify_token(refresh_token)
        if not payload or payload.get("type") != "refresh":
            return None

        await self.revoke_token(refresh_token)

        user_data = {"sub": payload.get("sub"), "type": "access"}
        return await self.create_access_token(user_data)

    async def revoke_token(self, token: str) -> bool:
        try:
            self._blacklisted_tokens.add(token)
            return True
        except Exception:
            return False

    async def is_token_revoked(self, token: str) -> bool:
        return token in self._blacklisted_tokens

    def get_token_payload(self, token: str) -> Optional[Dict[str, Any]]:
        """
        Obtiene el payload de un token sin verificar expiración.
        """
        try:
            return jwt.decode(
                token,
                self._public_key,
                algorithms=[self.algorithm],
                options={"verify_exp": False},
            )
        except JWTError:
            return None
